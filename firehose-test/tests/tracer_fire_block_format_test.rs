...
use firehose::{BlockData, BlockEvent, FinalizedBlockRef, FlashBlockData};
use firehose_test::{
    hash32, miner_addr, test_block, FirehoseBlockEntry, TracerTester,
};
...
fn test_fire_block_format_lib_num_empty_finality() {
    ...
    for (block_number, expected_lib_num) in cases {
        ...
        tester
            .tracer
            .on_block_start(BlockEvent::new(bd.clone()));
        ...
        let prev_num = if block_number == 0 { 0 } else { block_number - 1 };
        assert_all_fields(
            &entries[0],
            block_number,
            0,
    ...
    for (block_number, finalized_number, expected_lib_num) in cases {
        ...
        tester.tracer.on_block_start(
            BlockEvent::new(bd.clone())
                .with_finalized(FinalizedBlockRef { number: finalized_number, hash: None }),
        );
        tester.tracer.on_block_end(None);
...
fn test_fire_block_format_lib_num_clamped_at_200() {
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd.clone())
            .with_finalized(FinalizedBlockRef { number: 100, hash: None }),
    );
    tester.tracer.on_block_end(None);
...
fn test_fire_block_format_lib_num_across_multiple_blocks() {
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd300.clone())
            .with_finalized(FinalizedBlockRef { number: 250, hash: None }),
    );
    tester.tracer.on_block_end(None);
    ...
    assert_all_fields(
        &entries[0],
        300, 0, bd300.hash, 299, bd300.parent_hash, 250, bd300.time,
    );
    assert_all_fields(
        &entries[1],
        301, 0, bd301.hash, 300, bd301.parent_hash, 101, bd301.time,
    );
}
...
fn test_fire_block_format_flash_block_idx_non_flash() {
    ...
    assert_all_fields(
        &entries[0],
        bd.number, 0, bd.hash, bd.number - 1, bd.parent_hash, 0, bd.time,
    );
}
...
fn test_fire_block_format_flash_block_idx_partial() {
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd.clone())
            .with_flash_block(FlashBlockData { idx: 5, is_final: false }),
    );
    tester.tracer.on_block_end(None);
    ...
    assert_all_fields(
        &entries[0],
        bd.number, 5, bd.hash, bd.number - 1, bd.parent_hash, 0, bd.time,
    );
}
...
fn test_fire_block_format_flash_block_idx_final() {
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd.clone())
            .with_flash_block(FlashBlockData { idx: 10, is_final: true }),
    );
    tester.tracer.on_block_end(None);
    ...
    assert_all_fields(
        &entries[0],
        bd.number, 1010, bd.hash, bd.number - 1, bd.parent_hash, 0, bd.time,
    );
}
...
fn test_fire_block_format_flash_block_idx_sequence() {
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd.clone())
            .with_flash_block(FlashBlockData { idx: 1, is_final: false }),
    );
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd.clone())
            .with_flash_block(FlashBlockData { idx: 3, is_final: false }),
    );
    tester.tracer.snapshot_flash_block_for_next_iteration();
    tester.tracer.on_block_end(None);
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd.clone())
            .with_flash_block(FlashBlockData { idx: 5, is_final: true }),
    );
    tester.tracer.on_block_end(None);
    ...
    assert_all_fields(
        &entries[0],
        bd.number, 1, bd.hash, bd.number - 1, bd.parent_hash, 0, bd.time,
    );
    assert_all_fields(
        &entries[1],
        bd.number, 3, bd.hash, bd.number - 1, bd.parent_hash, 0, bd.time,
    );
    assert_all_fields(
        &entries[2],
        bd.number, 1005, bd.hash, bd.number - 1, bd.parent_hash, 0, bd.time,
    );
}
...
fn test_fire_block_format_finality_and_flash_block() {
    ...
    tester.tracer.on_block_start(
        BlockEvent::new(bd.clone())
            .with_finalized(FinalizedBlockRef { number: 480, hash: None })
            .with_flash_block(FlashBlockData { idx: 7, is_final: true }),
    );
    tester.tracer.on_block_end(None);
