...
fn new_flash_block_event(idx: u64) -> BlockEvent {
    ...
    event.flash_block = Some(FlashBlockData {
        idx,
        is_final: false,
    });
    event
...
fn new_flash_block_event_from_block(block: BlockData, idx: u64) -> BlockEvent {
    BlockEvent {
        block,
        finalized: None,
        flash_block: Some(FlashBlockData {
            idx,
            is_final: false,
        }),
    }
...
fn flash_tx1() -> TxEvent {
    flash_tx(1, 0x11)
}
fn flash_tx2() -> TxEvent {
    flash_tx(2, 0x22)
}
fn flash_tx3() -> TxEvent {
    flash_tx(3, 0x33)
}
fn flash_tx4() -> TxEvent {
    flash_tx(4, 0x44)
}
...
fn test_flash_block_basic_handling() {
    ...
    assert_eq!(
        blocks[1].transaction_traces.len(),
        4,
        "second flash block should have 4 transactions"
    );
...
fn test_flash_block_sequence_validation_same_index_panics() {
    ...
    tester
        .tracer
        .on_block_start(new_flash_block_event_from_block(block1_data.clone(), 1));
    tester.tracer.snapshot_flash_block_for_next_iteration();
    tester.tracer.on_block_end(None);
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        tester
            .tracer
            .on_block_start(new_flash_block_event_from_block(block1_data, 1));
    }));
...
fn test_flash_block_sequence_validation_backwards_index_panics() {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        tester
            .tracer
            .on_block_start(new_flash_block_event_from_block(block1_data, 0));
    }));
...
fn test_flash_block_sequence_validation_new_block_number_no_panic() {
    ...
    tester
        .tracer
        .on_block_start(new_flash_block_event_from_block(block2, 0));
    tester.tracer.on_block_end(None);
}
...
fn test_flash_block_sequence_validation_non_sequential_but_higher() {
    ...
    tester
        .tracer
        .on_block_start(new_flash_block_event_from_block(block1_data, 4));
    tester.tracer.on_block_end(None);
}
...
fn test_flash_block_persists_on_regular_block() {
    ...
    assert!(
        tester.tracer.has_flash_block_snapshot(),
        "snapshot should persist through regular block"
    );
...
fn test_flash_block_snapshot_basic_usage() {
    ...
    assert_eq!(
        blocks[1].transaction_traces.len(),
        3,
        "second block should have tx1+tx2 (from snapshot) + tx4 (new)"
    );
    assert_eq!(blocks[1].transaction_traces[0].nonce, 1);
    assert_eq!(blocks[1].transaction_traces[1].nonce, 2);
    assert_eq!(blocks[1].transaction_traces[2].nonce, 4);
    assert_eq!(
        blocks[1].system_calls.len(),
        1,
        "second block should have only 1 system call from snapshot"
    );
}
...
fn test_flash_block_snapshot_without_snapshot() {
    ...
    assert_eq!(
        blocks[1].transaction_traces.len(),
        0,
        "second block should start fresh with no transactions"
    );
}
...
fn test_flash_block_snapshot_cleared_on_new_block() {
    ...
    tester
        .tracer
        .on_block_start(new_flash_block_event_from_block(block2, 1));

    // Snapshot should have been cleared because the block number changed
    assert!(
        !tester.tracer.has_flash_block_snapshot(),
        "snapshot should be cleared on new block number"
    );
    ...
    assert_eq!(
        blocks[1].transaction_traces.len(),
        0,
        "second block should start fresh after snapshot is cleared"
    );
}
...
fn test_flash_block_snapshot_system_calls_included() {
    ...
    assert_eq!(
        blocks[1].system_calls.len(),
        2,
        "only system calls before the snapshot should appear"
    );
}
...
fn test_flash_block_snapshot_balance_changes_included() {
    ...
    assert_eq!(
        blocks[1].balance_changes.len(),
        1,
        "only balance change before snapshot"
    );
    assert_eq!(
        blocks[1].balance_changes[0].address,
        miner_addr().as_slice()
    );
}
...
fn test_flash_block_snapshot_code_changes_included() {
    ...
    assert_eq!(
        blocks[1].code_changes.len(),
        1,
        "only code change before snapshot"
    );
    assert_eq!(
        blocks[1].code_changes[0].address,
...
fn test_flash_block_snapshot_on_non_flash_block_is_noop() {
    ...
    assert!(
        !tester.tracer.has_flash_block_snapshot(),
        "no snapshot should be created for non-flash blocks"
    );
