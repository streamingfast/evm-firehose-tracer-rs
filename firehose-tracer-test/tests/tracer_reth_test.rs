...
use firehose_tracer::pb::sf::ethereum::r#type::v2::TransactionTraceStatus;
use firehose_tracer_test::{alice_addr, bob_addr, failed_receipt, test_legacy_trx, TracerTester};
...
fn test_reth_reconciliation_does_not_overwrite_existing_failure() {
    let err = firehose_tracer::types::StringError("execution reverted".to_string());
    let mut tester = TracerTester::new_reth();
    tester
        .start_block_trx(test_legacy_trx())
